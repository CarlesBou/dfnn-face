...
Deep feedforward neural networks (DFNNs) have achieved remarkable success across numerous domains, but their internal decision-making process remains largely opaque, limiting transparency and trust in high-stakes applications. Existing explainability approaches typically rely on approximate techniques and lack an exact symbolic characterization of the network behavior.

This paper introduces R-FACE, a framework that constructs an exact symbolic representation of the behavior of piecewise-linear (PWL) DFNNs over a user-defined set of interest (SOI). Rather than exhaustively characterizing the network's global partition, R-FACE identifies the activation regions induced by the SOI. The resulting representation consists of a collection of exact IF-THEN rules, each encoding the polyhedral constraints that define a single region (antecedent) together with the corresponding exact affine output model and feature attributions (consequent). 

The proposed framework is built upon a compact closed-form matrix formulation and extends to other PWL activation functions, including Leaky ReLU, hard sigmoid, and hard tanh. Once constructed, the resulting symbolic representation enables exact analytical queries over PWL DFNN behavior beyond explainability, such as constrained optimization and adversarial example generation. Representative use cases illustrate the construction, versatility, and analytical utility of the proposed framework.


# Introduction

R-FACE is a regional explainability framework designed for piecewise-linear deep feedforward neural networks. Rather than approximating the network globally, it constructs an exact symbolic representation of the network's input-output behavior over a user-defined set of interest (SOI). It serves as the natural extension of our local explainability method, FACE (Feature Attribution Computed Exactly), which computes exact local feature attributions by leveraging the network's underlying activation regions. By identifying the specific regions induced by the SOI, R-FACE extracts exact IF–THEN rules that encode both the polyhedral constraints and the corresponding affine output models. 
